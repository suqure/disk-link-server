...
@Slf4j
public class FileUploadMessageTask implements Runnable {
	...
	public FileUploadMessageTask(ShareFile file, String device, int chunkSize, long startChunk, long maxBuffer) {
	...
	public FileUploadMessageTask(ShareFile file, String device) { 
		this(file, device, null); 
	}

	public FileUploadMessageTask(ShareFile file, String device, ChunkReadListener listener) {

		this(file, device, 0, 0, 0, listener);

	}
	
	public FileUploadMessageTask(ShareFile file, String device, int chunkSize, long startChunk,
			ChunkReadListener listener) {
		...
		try {
			...
			while (true) {
				...
				if (channel.getBufferedAmount() < maxBuffer) {
					...
					info.setChunk(Base64.encodeBase64String(buffer)); 
					channel.send(ClientMessageUtil.buildChannelBuffer(ClientMessageUtil.buildChannelMessage(
							ChannelMessageType.FILE, ClientMessageUtil.buildFileData(ChannelFileType.CHUNK, info))));
					if (listener != null) {
