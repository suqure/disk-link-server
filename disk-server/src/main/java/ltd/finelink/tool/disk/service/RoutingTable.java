...
public class RoutingTable {
	...
	public static class TrieNode {
		...
		// Bucket:存储最多8个Node
		private NodeInfo[] nodes = new NodeInfo[MAX_NODE_NUM];
		// 锁id
		 ...
		 */
		public NodeInfo contain(byte[] nodeId) {
		...
		public void split() {
			...
			// 将原来的所有节点分配到新节点
			for (NodeInfo itemNode : this.nodes) {
				// 获取itemNode的下一位二进制值
	 ...
	 * @param config
	 */
	public RoutingTable(byte[] nodeId, String ip, int port) {
		// 参数
		this.nodeId = nodeId;
		maxStorePrefixLen = 10;
		lockNum = 10;
		...
		// 存入主节点(自己的nodeId)
		put(new NodeInfo(this.nodeId, ip, port, Integer.MAX_VALUE));
	}

	/**
	 * 新增若干节点
	 */
	public void putAll(Collection<NodeInfo> nodes) {
	 ...
	 * 新增节点
	 */
	public boolean put(NodeInfo node) {
	...
	public TrieNode get(byte[] nodeId) {
		...
		try {
			for (int i = 0; i <= MAX_PREFIX_LEN; i++) {
				...
				try {
					lock(currentNode.lockId);
					NodeInfo node = currentNode.contain(nodeId);
	 ...
	 */
	public List<NodeInfo> getForTop8(byte[] nodeId) {
			...
			for (int i = 0; i <= MAX_PREFIX_LEN; i++) {
				...
				try {
					...
					if (findNode.count != 0)
						...
						nodes.addAll(Arrays.asList(findNode.nodes).subList(0,
								lackNum <= findNode.count ? lackNum : findNode.count));
					return nodes;
				} finally {
	...
	private void loop(TrieNode node, Consumer<TrieNode> consumer) {
		if (node.next[0] != null)
			loop(node.next[0], consumer);
		if (node.next[1] != null)
			loop(node.next[1], consumer);
		if (node.count > 0) {
			try {
				lock(node.lockId);
	...
	/**
	 * 遍历节点,并使用指定函数 操作nodes 封装,去除一个参数
	 */
