...
public class CodeUtil {
    ...
    public static byte[] bytesXorBytes(byte[] bytes1, byte[] bytes2) {
        if (bytes1.length != bytes2.length)
            throw new BizException("异或计算,两个byte[]长度不相同.byte1.len:+" + bytes1.length + "+,byte2.len:" + bytes2.length);
    ...
    public static int xorResultComparexxxx(byte[] bytes1, byte[] bytes2) {
        if (bytes1.length != bytes2.length)
            throw new BizException("异或结果比较,两个byte[]长度不相同.byte1.len:+" + bytes1.length + "+,byte2.len:" + bytes2.length);
    ...
    public static String bytes2Ip(byte[] ipBytes) {
        if (ipBytes.length != 4) {
            throw new BizException("bytes2Ip失败,bytes长度不为4.当前长度:" + ipBytes.length);
        }
    ...
    public static byte[] ip2Bytes(String ip) {
        if (StringUtils.isBlank(ip)) {
            throw new BizException("ip2Bytes失败,ip为空");
        }
        String[] ips = ip.split("\\.");
        if (ips.length != 4) {
            throw new BizException("ip2Bytes失败,ip的段数长度不为4.ip:" + ip);
        }
        return new byte[]{
    ...
    public static int bytes2Port(byte[] portBytes) {
        if (portBytes.length != 2) {
            throw new BizException("bytes2Port失败,bytes长度不为2.当前长度:" + portBytes.length);
        }
